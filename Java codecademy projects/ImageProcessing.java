...
public class ImageProcessing {
    ...
    twoDToImage(shrinked, "./shrinked_kitten.jpg");

    int[][] inverted = invertImage(imageData);
    twoDToImage(inverted, "./inverted_kitten.jpg");

    int[][] filtered = colorFilter(imageData, -100, +30, +170);
    twoDToImage(filtered, "./filtered_kitten.jpg");

    int[][] canvasRandom = new int[500][500];
    int[][] randomImage = paintRandomImage(canvasRandom);
    twoDToImage(randomImage, "./random_image.jpg");

    int[] rgba = {200, 200, 255 , 255};
    int[][] rectangle = paintRectangle(imageData, 100, 150, 50, 0, getColorIntValFromRGBA(rgba));
    twoDToImage(rectangle, "./rectangle.jpg");

    int[][] randomRectangles = generateRectangles(randomImage, 1000);
    twoDToImage(randomRectangles, "./random_rectangles.jpg");
    ...
    for (int i = 0; i < imageTwoD.length; i++) {
      for (int j = 0; j < imageTwoD[0].length; j++) {
        doubleJ = j * 2;
        stretchedImage[i][doubleJ] = imageTwoD[i][j];
        stretchedImage[i][doubleJ + 1] = imageTwoD[i][j];
    ...
    for (int i = 0; i < canvas.length; i++) {
      for (int j = 0; j < canvas[i].length; j++) {
       int red = number.nextInt(256);
       int green =number.nextInt(256);
       int blue = number.nextInt(256);
    ...
    for (int i = 0; i < canvas.length; i++) {
      for (int j = 0; j < canvas[i].length; j++) {
        if (i > rowPosition && i <= rowPosition + width) {
          if (j > colPosition && i <= colPosition + height) {
            canvas[i][j] = color;
          }
        }
      }
    ...
    for (int i = 0; i < numRectangles; i++) {
        ...
        int randomWidth = randomNo.nextInt(canvas[0].length);
        int randomHeight = randomNo.nextInt(canvas.length);
        int rowPosition = randomNo.nextInt(canvas.length - randomHeight);
        int colPosition = randomNo.nextInt(canvas[0].length - randomWidth);

        int red = randomNo.nextInt(256);
        int green = randomNo.nextInt(256);
        int blue = randomNo.nextInt(256);

        int[] rgba= {red, green, blue, 255};
        int randomColor = getColorIntValFromRGBA(rgba);

        canvas = paintRectangle(canvas, randomWidth, randomHeight, rowPosition, colPosition, randomColor);
      }
